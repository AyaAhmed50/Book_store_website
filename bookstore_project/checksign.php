<?php
    ...
    if(mysqli_num_rows($check) == 1){
        // $row = $res -> fetch_assoc();
        header("Location: Signup.php?error=you have already an account");
        // if(($row['User_Name'] == $name && $row['password']==$pass) && $row['email'] = $Email){
        //     header("Location: Signup.php?error=you have already an account");
        // }
        // else{
        ...
        $stmt = $connection->prepare("insert into accounts(User_Name,password,email) values(?,?,?)");
...
<body>
    <div class="content">
        <div class="center">
            ...
            <button><a href="login.php">you can Login</a></button>
        </div>
    </div>
